...
| Cq4 | Results go to the enqueuer, not the runner — cross-pack delivery is orch↔orch messaging | `BUILT` — the event is keyed to the enqueuer (`src/daemon/work-loop.ts:163` `key: task.enqueuedBy`), and the RESULT is now actually delivered: `src/daemon/result-delivery.ts:21` `deliverTaskResult` writes it to the enqueuer's `inbox.jsonl` through the one existing writer (`appendPeerInbox`, `src/agent/peers.ts:210`), wired at all three settlement sites in `src/daemon/work-loop.ts` (done x2, failed x1). Cross-pack needs it: there is no shared parent to hand a result up through, and an event stream is only read by whoever happens to be watching. Proven by `test/cross-pack-result-delivery.test.ts` — 4 tests: a foreign-pack enqueuer receives it and the runner does not, the payload travels (not just a notification), a FAILURE reports back, and an enqueuer with no presence dir neither throws nor unsettles the task. |
...
| D7 | Kill path never routes through the leasing orch and never touches the plexer | `BUILT` — **the lease never gates ending**: `assertNotHeldByLiveForeignOrch` (`src/commands/lease.ts:133`) has exactly three call sites — detach `:117`, adopt `:156`, rename `:169` — and none is close/abort/reap, so an orch can close its own slave while another orch drives it and a dead holder can never keep a runaway alive. **The plexer is not required**: an unknown or absent backend logs `close.unknown-backend` and still reaps the record (`src/commands/lifecycle.ts:527`). **OWNERSHIP does gate it for an agent** (ruling, `TASKS/01-agent-model.md` *Who may end an agent*): `src/policy/close-authority.ts` — the human is unrestricted, an agent reaches only its own provenance subtree at any depth and may always end itself; wired at `src/commands/lifecycle.ts:509` (caller), `:524` (`--all` sweeps only what the caller owns) and `:548` (a named target is refused, naming the owner). Proven by `test/close-authority.test.ts` — 7 tests incl. "the LEASE never decides it: a foreign holder does not block the owner" and a provenance cycle terminating. |
| D8 | Adoption announces itself when a session starts where unleased agents exist | `BUILT` — `hello` carries the adoptable summary (`src/daemon/rpc.ts:33` `HelloResponse.unleased`, built at `:305` `unleasedAgents`), and a starting session announces it at `:198`: `"<n> unleased agent(s) exist - orch adopt <name> to take one, orch status to see them."` Silent when there are none (`:198` early-returns on an empty list). Proven by `test/commands-index.test.ts:20`. |
