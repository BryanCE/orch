...
| D7 | Kill path never routes through the leasing orch and never touches the plexer | `BUILT` — matches `TASKS/01-agent-model.md:342` (`abort`/`close`/`reap`, gated: **no** — "ending. Nothing to interleave with, and the human must always be able to stop a runaway agent from the CLI or the web") and CLAUDE.md Rule 11. The live-foreign-holder gate `assertNotHeldByLiveForeignOrch` (`src/commands/lease.ts:133`) has exactly three call sites — detach `:117`, adopt `:156`, rename `:169` — and none is close, abort or reap; `src/commands/lifecycle.ts:539` states it. The plexer half holds too: an unknown or absent backend logs `close.unknown-backend` and still reaps the record (`:524`). |
