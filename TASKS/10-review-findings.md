...
special-cases out-of-date data, and Rule 14 states flatly that NOTHING IS PUBLISHED — so there is no
older bridge, no older install, and no phase-1 consumer for any of these to serve. In every case the
justifying COMMENT is the tell:
- `src/doctor/presence.ts:55-63` — a record failing the `PRESENCE_SCHEMA` gate is kept out of the
  verdict when its process is alive, for "a live session on older bridge code".
- `src/types/doctor.ts:20-21` — `available?: boolean`, "Legacy spelling retained".
- `src/extensions/bundles.ts:13-14` `RETIRED_EXTENSION_NAMES` + its reaping loop at
  `src/adapters/pi.ts:330-332`, and the sibling `.ts`-link removal at `:329`, for "older installs".
- `src/types/notify.ts:55-56` — `readonly (NotifierConfigField | string)[]`, a union accepting TWO
