...
|---|---|---|---|
| B1 | `13-logging.md` slice 4 | Every `process.stderr.write` under `src/` is gone except the logger's own sink guard. Each site becomes a `log.<level>(event, fields)` call at the right level, or — where it is genuinely output the human asked for — a `process.stdout.write`. Decided per site. A static test asserts zero `process.stderr.write` in `src/` and `extensions/`. | `BUILD` |
