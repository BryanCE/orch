...
| B2 | `07-port-seam.md` slices 5 + 9 | The `Backend` shell is deleted: no `Backend` interface, no `workspaceNames()`, no `BackendWorkspace`, no "workspace" vocabulary in `src/types/backend.ts` or anywhere in orch's core (it is a plexer's word — orch says space/pack and stores the plexer coordinate). `checkBackendCapabilities` in `src/doctor/backends.ts` becomes an environment description, not a capability projection. A static test forbids the deleted names. | `BUILT` — the shell members are gone: `workspaceNames()` deleted from the port (src/types/backend.ts) and from herdr, tmux and headless providers and the test fake; `BackendWorkspace` deleted, herdr maps its workspace list straight onto `PlexerHome` for spaceHome.list. The doctor no longer projects capabilities: `describeBackendEnvironments` (src/doctor/backends.ts, check id backend-environments) reports which composed roles each enabled backend holds by reading the role fields, never a backend id. What remains of `interface Backend` is the composed-roles port itself (id, isAvailable, isInsideSession, spawn, plus role fields), with zero optional methods and no capability flags. The port field named `workspace` is the plexer coordinate by contract (ADR 0001) and stays. Proven by test/port-has-no-shell.test.ts (static: neither name in the port nor anywhere under src/) and the doctor tests covering backend-environments. |
| B3 | `14-settings-tui.md` slice 6 | `src/commands/settings.ts` writes NOTHING except through `writeRegisteredSetting`. The direct `writeSettings*` calls (`writeSettingsDefault`, `writeSettingsModels`, `writeSettingsAllowedModels`, `writeSettingsPreferredModels`, `writeSettingsSkills`, `writeSettingsNotify`, `writeSettingsThinking`) are deleted from that file; any writer that then has no importer is deleted from `src/config.ts`. A test asserts the subcommands persist through the registry (spy on the registry, not on the file). | `BUILT` — `src/commands/settings.ts` imports no `writeSettings*`; `--harness`/`--plexer` switch through `writeRegisteredSetting(orchDir(), "defaults.adapter" or "defaults.backend", …)` (`src/commands/settings.ts:162-163`). The registry owns the validation: `writeDefaultAdapter`/`writeDefaultBackend` in `src/settings/registry.ts` narrow with `isAdapterId`/`isBackendId` and persist via `writeSettingsDefault`, which still refuses an id that is not installed — so the refusal is unchanged and lives in ONE place. `writeSettingsDefault` stays in `src/config.ts` because `src/commands/setup.ts` imports it. The single-setting set path (`orch settings <key> <value>`) also goes through `writeRegisteredSetting` (follow-up `69e8bc0`). Proven by `test/settings-command.test.ts` (registry-path switch, a `spyOn(registry, "writeRegisteredSetting")` assertion on the single-set path, and the static assertion that `settings.ts` names no writer but `writeRegisteredSetting`). |
