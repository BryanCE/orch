...
| T3 | Worker prompt: the "never spawn subagents" clause becomes conditional on `depthOf(caller) < fleet.max_depth`, passed as `WorkerHeaderContext.maySpawn`. Today the clause is unconditional, which is *correct* at the default of 1 and *wrong* the moment a user raises it. Needs the spawner's depth at prompt-compose time in `spawn.ts:488`, `control.ts:202`, `lifecycle.ts:51`. | open |
