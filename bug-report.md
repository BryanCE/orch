...
## 2026-08-28 ~15:43 / ~17:04 / ~17:05 — `orch spawn` starts NO agent: herdr "timed out waiting for agent startup" for pi AND claude (t3reports session, herdr 0.8.2, orch 0.1.0 npm install)
- Attempt 1 (15:43, orchd not running): `orch spawn 3 --name c11 --cwd <repo>` → `root spawn failed: herdr agent start c11-1 --kind pi --pane wF:p73 failed: spawnSync herdr ETIMEDOUT`. herdr-server.log 9773-9792: ten `agent.start` requests inside ~1 s, each `outcome="error"` in <2 ms, then one `ok`; the herdr CLI call hung until orch's spawnSync timeout.
- Attempt 2 (17:04, orchd pid 37709 running, `orch clean` done): same command → `herdr agent start research-1 --kind pi --pane wF:p77 --timeout 30000 failed: exit status 1; {"error":{"code":"timeout","message":"timed out waiting for agent startup"}}`. Server log: `agent.start ok` 17:04:38.166 → `request_id="cli:agent:start:timeout" method="agent.get" outcome="error"` 17:05:05. herdr accepted the start and created the pane; the agent never became "ready" inside 30 s; orch tore the pane down (`herdr agent list` shows only the orchestrator pane afterwards).
- Attempt 3 (17:05, `--agent claude`, pane wF:p79): identical timeline (`ok` 17:05:30.808 → timeout 17:06:00.829). Not harness-specific: whatever herdr's readiness signal is, neither adapter produces it now. Compare 2026-08-27 "spawn/registration bug RESOLVED" (pi-bridge.js load) — this is a regression or a new readiness path.
- Fallback `--backend tmux` refused: `Backend "tmux" requires running inside a live tmux session`. An orchestrator living in a herdr pane has no second pane backend when herdr readiness breaks; only headless (no pane, no steer) remains. Suggest: spawn should surface WHAT it waited for (bridge presence dir? herdr agent_status?) and leave the pane open for inspection on timeout instead of tearing it down — the evidence died with the pane.
- Side noise: orchd idle-SIGTERMed twice between attempts (orchd.log 16:43:01, 16:52:05); `orch doctor` says "orchd is absent (daemon is optional)" while spawn then needs it up. Not causal (attempts 2-3 ran with it up) but it cost a restart per retry.
- Whole session ran on the built-in Claude Agent tool instead. Third session in a row where the orchestrator could not use orch for its first dispatch.
