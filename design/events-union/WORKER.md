...
You are one worker in a fleet turning orch's event into a discriminated union. You have one task, below this sheet. Do exactly it. Do not research beyond the files the task names, do not widen scope.
...
- The discriminant is `type`. Never introduce `kind`, `tag`, or a second spelling for the same fact.
- Narrow by `switch (event.type)` with a `default: { const exhaustive: never = event; ... }` arm, or by `"field" in event`. Never by a hand-written guard where the discriminant already narrows.
