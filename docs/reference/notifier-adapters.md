...
```json
{ "id": "herdr", "on": ["blocked", "error", "done"] }
```
...
```json
{ "id": "webhook", "url": "https://example.test/orch-events", "on": ["blocked", "error"] }
```
...
```json
{ "id": "command", "command": ["/usr/local/bin/orch-notify"], "on": ["blocked", "error", "done"] }
```
...
Each `notify` object maps to one adapter through `id`. Keys other than `id` and `on` become that adapter's configuration. Multiple objects configure multiple sinks:

```json
{
  "notify": [
    { "id": "desktop" },
    { "id": "webhook", "url": "https://example.test/orch-events" },
    { "id": "command", "command": ["logger", "-t", "orch"] }
  ]
}
```
