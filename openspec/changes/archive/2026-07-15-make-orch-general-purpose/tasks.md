...
- [x] 3.4 Claude Code adapter: hooks shim (SessionStart/Stop/Notification → presence writes), `orch setup` merges hooks additively, interactive + headless commands, steer via keys fallback with warning *(2026-07-14: scripts/claude-hooks.ts schema-2 presence shim; src/adapters/claude.ts registered (caps steer:keys, ask:false); cmdSetup additively + idempotently merges the 3 hooks into ~/.claude/settings.json; test/claude-adapter.test.ts. Only 3.5 e2e-in-real-pane remains)*
- [x] 3.5 Verify claude adapter end-to-end in a herdr pane: spawn, dispatch, status transitions, result extraction
...
- [x] 3.7 Codex adapter per spike outcome, with honest caps (coarse states acceptable, `stateFallback` marker) *(2026-07-14: src/adapters/codex.ts — notify-hook (`agent-turn-complete`) primary state detection, coarse states w/ `CODEX_STATE_FALLBACK_MARKER`, layered result extraction; registered in commands.ts adapters registry; test/codex-adapter.test.ts)*

- [x] 3.9 STRICT WORKSPACE WALLS (OPERATOR ORDER 2026-07-13, restated twice — this is a hard product principle) *(DONE 2026-07-14, commit 359d7b9: `src/entities.ts` workspace helpers (`workspaceOf`/`entityWorkspace`/`currentWorkspace`/`scopeEntitiesToWorkspace`); PULL views `status`/`panes`/`questions`/`tabs` walled to current workspace w/ `--all` opt-in + workspace-labeled cross-wall rows; bridge peer tools `orch_agents`/`orch_send`/`orch_read` same-workspace-default w/ `all_workspaces`/`cross_workspace` overrides, fail-closed; queue/work idle selection workspace-scoped (`nextQueuedTask` + `workspaceOf(entry.key)`); `logPath()`→ORCH_DIR via store resolver; notify+events carry `workspace`+per-workspace color, event line reuses shared formatter. PUSH channel (events/toasts) stays cross-workspace with provenance per the notifications exception. Follow-ups tracked separately: notifier-adapter refactor + bridge-toast format consistency)*: agents/orchestrators operate ONLY within their workspace, and EVERY read surface defaults to the current workspace — `orch status`, `questions`, `events`, `tabs`, `result`, all of it shows current-workspace rows only, `--all` opt-in for cross-workspace views; bridge peer tools `orch_agents`/`orch_send`/`orch_read` same-workspace only with explicit override; queue/work-engine idle-agent selection workspace-scoped. Any row or payload that DOES cross the wall (`--all` views, toasts) must carry unmistakable workspace identity — name prefix + per-workspace color — so nothing from two fleets is ever visually confusable. **EXCEPTION — notifications (toasts) DO cross workspaces**: the user wants every fleet's toasts, but each must be color-coded and carry the origin workspace/agent name so provenance is obvious at a glance. Observed live twice: a w8 worker sent its completion report to a w1 pane (foreign turn burned), and the daemon work loop assigned a w8-queued task to idle w1:p1K (orchd live-verify, 2026-07-13). Also found: lifecycle `logPath()` falls back to `process.cwd()` instead of ORCH_DIR — daemon log lands in whatever dir started it; fix alongside
- [x] 3.10 **Agent-teams is NOT an orch feature** (operator ruling 2026-07-13, final) *(DONE 2026-07-14: generalized — Adapter interface gains optional `restrictedInteractiveCmd?`/`restrictedHeadlessCmd?` (adapters that can't restrict omit them, NOT pi-hardcoded); pi impl launches workers with an explicit tool allowlist `PI_APPROVED_TOOLS` = read/write/edit/bash + the four `orch_*` bridge tools via `pi --tools <list> --no-builtin-tools`; spawn path uses the restricted cmd by default, `--cmd` override preserved. Follow-up: empirically confirm the `--tools`+`--no-builtin-tools` combo fully blocks non-allowlisted extension tools on next real spawn)*: pi agent-teams is a separate project, not prod-ready, and orch agents were only using it because pi auto-discovers global extensions — worker panes inherited it and self-delegated into invisible in-process children (no pane/presence/status/toast visibility, unattributable cost, sleep-poll babysitter parents, cross-workspace team-config leakage). **Disabled globally 2026-07-13** (moved to ~/.pi/agent/extensions-disabled/). Durable product fix: `orch spawn` launches workers with an explicit extension ALLOWLIST (bridge + herdr-agent-state + approved set) instead of trusting global discovery, so nothing installed later can silently enter worker panes. If agent-teams ever returns, it needs its own openspec change meeting the 3.9 walls + full presence visibility first
...
- [x] 4.4 Backend auto-selection (herdr when present, else headless) + `--backend` flag; herdr-only commands (zoom/peek/keys/move/focus) fail with clear message on headless agents *(2026-07-14)*
- [x] 4.5 Verify full no-herdr loop on a PATH without herdr: spawn → status → steer → result → close; `orch events` streams transitions for headless agents
...
- [x] 6.3 Interactive `orch review` (a/r/s loop with diff display) as sugar over the plumbing *(2026-07-14: bare `orch review` enters a stdin a/r/s loop over pending reviews with diff display; `review list` + non-interactive approve/reject unchanged)*
...
- [x] 6.5 End-to-end: 2 worktree agents, one approve (merged + cleaned), one reject → iterate → approve
...
- [x] 7.2 Ship the WSL toast bridge script; wire `orch events --notify` and auto-attach in `orch work` *(bridge script + `events --notify` done 2026-07-13; auto-attach DONE 2026-07-14 — runWorkLoop loads sinks once and delivers per-poll state transitions best-effort, sink failure never breaks dispatch)*
...
- [x] 8.1 Implement `src/doctor.ts` checks: bins, extension symlinks/currency, claude hooks shim currency, stale presence dirs, registry consistency, herdr version, config validity, worktree gitignore, desktop-notification chain; non-zero exit on failure *(module + tests + `orch doctor` CLI done 2026-07-13; claude-hooks-currency check added 2026-07-14 once 3.4 shipped — verifies ~/.claude/settings.json hooks present + pointing at current shim, warns 'run orch setup' if stale)*
- [x] 8.2 `orch doctor --fix` for reversible fixes only, listing every change made
- [x] 8.3 Replace `orch setup`'s ad-hoc checks with doctor calls where they overlap *(2026-07-14: cmdSetup routes overlapping checks through shared src/doctor.ts check functions)*
...
- [x] 9.2 Parallel fan-out for `status`/`questions` with HOST column, warning rows for dead hosts, `--local` flag; route write commands (steer/answer/dispatch/queue add --host) through host prefix *(2026-07-14: parallel runSSH fan-out + local, merged rows w/ HOST column, warningStatusRow for dead hosts, --local flag, remoteWrite host-prefix routing via parseTarget)*
- [x] 9.3 Doctor host onboarding checks (reachability, remote orch + version/schema match, remote ORCH_DIR) with copy-paste fixes *(2026-07-14: per-host checks via runSSH — reachability/version/ORCH_DIR — with copy-paste fix hints, injectable ssh runner, no-hosts fallback; test/doctor-hosts.test.ts)*
- [x] 9.4 Audit: every observe/control command supports `--json` and runs TTY-free (orchd-readiness gate); fix any stragglers *(2026-07-14: added --json to panes/tabs/workspaces/tail/session/peek/work + answer/steer/pipe/broadcast/run/dispatch/model/wait + pane/tab/spawn/tile/clean/daemon actions; TTY-free confirmed except interactive `orch review` which has scripted list/approve/reject fallback. NOTE: repo carries ~77 pre-existing strict-tsc errors from commit dcaa78b — separate type-debt cleanup, not this task; functional gates (bun test + smoke) green)*
- [x] 9.5 End-to-end against a second machine (or localhost-as-host): remote spawn headless, merged status, remote steer/result, doctor all green *(2026-07-15: localhost SSH loop verified end-to-end with temporary ORCH_DIR; remote headless spawn, merged status, steer/result round-trip, and doctor host checks all green)*

## 10. Ship

- [x] 10.1 README rewrite: adapters, backends, queue, review, notifications, config reference, remote hosts, orchd roadmap note *(2026-07-14: substantial draft shipped — adapters/backends/queue/review/notifications/workspace-walls/config/doctor; remote-hosts as roadmap; final polish at publish 10.2)*
- [x] 10.2 Full smoke suite green on WSL2 and a herdr-less Linux box
