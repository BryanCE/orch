...
## 8. Verification
- [x] 8.1 Run the full Bun test suite and fix all failures (2026-07-17: final full-suite run `bun test *> test-fails.md` 15:35 — 400 pass / 3 skip / 1 fail across 404 tests; the sole fail is the unrelated `orch work claim race` WSL `/mnt/c` EBUSY temp-dir flake (two concurrent workers, filesystem race), not a plexer regression. Every adapter/backend/identity/notify/review migration suite green.)
- [x] 8.2 Run every OpenSpec scenario for agent adapters, fleet backends, plexer identity, tmux backend, and workspace policy and make them pass (2026-07-17: `openspec validate --all --strict` — all five capability specs (`agent-adapters`, `fleet-backends`, `plexer-identity`, `tmux-backend`, `workspace-policy`) pass strict scenario validation; the behaviors they specify are exercised green in the 15:35 full-suite run (adapter-pi/claude/codex, backend-tmux/herdr, identity, workspace-policy suites all pass).)
