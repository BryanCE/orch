...
- [x] 5.2 With a daemon running, `orch events` streams transitions; kill the daemon mid-stream and confirm it exits non-zero naming `orch daemon start` rather than continuing to emit. (2026-07-20: `daemon presence events > a dead daemon closes the subscription instead of falling back to files` proves the close handler fires on daemon death and that there is no file-watch fallback, and `> a caller-initiated stop is not reported as a disconnect` proves it does not misfire on clean exit. The remaining piece — that the exit names the recovering command — had no coverage, so the message is now the exported `DAEMON_DISCONNECTED` constant asserted by `commands/events > a dropped subscription names the command that recovers it`.)
