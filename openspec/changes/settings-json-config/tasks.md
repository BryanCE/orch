...
- [ ] 5.1 Only if a future writer needs multi-key/atomic writes: generalize `writeSettingsDefault` and add tmp+rename hardening. Not in scope otherwise — the shipped whole-file round-trip writer satisfies the read-modify-write intent (design D4).
