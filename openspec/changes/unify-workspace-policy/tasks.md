...
- [ ] 3.1 `src/config.ts`: add `[defaults] worker_peer_tools` (default false) with validation.
- [ ] 3.2 `src/commands.ts`: one `workerTools(config)` helper builds the `--tools` list
...
- [ ] 4.4 Verify in a real pane: spawn a worker (default config), confirm it CANNOT discover/message
...
- [ ] 5.2 Confirm re-introducing wall logic in a second file fails `bun check` (duplicate gate works).
...
- [ ] (e) **IN PROGRESS:** `commands.ts` any-typing, catch-block `errorMessage` sweep, and cross-platform test fixes.
