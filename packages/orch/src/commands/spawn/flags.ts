...
function readSpawnFlag(flags: SpawnFlags, args: string[], index: number): number {
  ...
  switch (argument) {
    ...
    case "--file": flags.promptFile = args[index + 1]!; return 1;
...
export function parseSpawnFlags(args: string[]): SpawnFlags {
  const flags: SpawnFlags = {
    ...
    space: null, promptFlags: [], withPaths: [], unknownFlags: [], positional: [],
  };
...
export type SpawnSettings = AgentSettings & {
  ...
  space: string | null;
  prefix: string;
  /** One name per agent, or a single name that numbers into "<prefix>-<n>". */
  names: string[];
  n: number;
  worktree: boolean;
  /** Initial tasks, mapped one-to-one for placed agents; headless agents run their task and exit. */
...
function resolveSpawnPrompts(flags: SpawnFlags, n: number): string[] {
  const sources = [flags.tasksFile !== undefined, flags.promptFile !== undefined, flags.promptFlags.length > 0].filter(Boolean);
