 ...
 */
export function spawnerIdentity(orchDir: string): SpawnerIdentity {
...
export function spawnerIsRepliable(orchDir: string): boolean {
  return spawnerIdentity(orchDir).key !== null;
}

/** The header context for a worker THIS session dispatches to. */
export function workerHeaderContext(orchDir: string, settings: OrchSettings): WorkerHeaderContext {
  return {
