...
export function mergeAgentStatus(
  ...
  return orm(orchDir).transaction((tx) => {
    const previous = tx.select().from(agentStatus).where(eq(agentStatus.agentId, agentId)).get();
    ...
    if (previous === undefined) {
      tx.insert(agentStatus).values(values).run();
    } else {
      tx.update(agentStatus).set(values).where(eq(agentStatus.agentId, agentId)).run();
    }
    const current = tx.select().from(agentStatus).where(eq(agentStatus.agentId, agentId)).get();
    if (current === undefined) throw new Error(`orch: status row for ${agentId} disappeared during merge`);
    return { previous, current };
  });
}

export function deleteAgentStatus(orchDir: OrchDir, agentId: string): void {
  orm(orchDir).delete(agentStatus).where(eq(agentStatus.agentId, agentId)).run();
}
