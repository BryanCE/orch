...
// Inject the CLI runner per backend instance so this suite's fake cannot leak to
// another suite; real argv building, parsing, and error wrapping stay under test.
...
describe("herdr and notification hardening", () => {
  test("uses a non-empty agent name and preserves shell command as one argv value", () => {
    ...
    const handle = backend.spawn(adapter, { cwd: "/tmp/work dir", workspace: "ws-test", key: "  ", orchDir: tempOrchDir("herdr-hardening-") });
  ...
  test("falls back to a valid name when the identity key contains herdr-invalid separators", () => {
    ...
    new HerdrBackend(createHerdrCli(fakeExecutor)).spawn(adapter, { workspace: "ws-test", key: "herdr~ws-test~ABC_123", orchDir: tempOrchDir("herdr-hardening-") });
  ...
  test("nameless notifications use a space label, never a bare pane key", () => {
    ...
    emitAndNotify((value) => { emitted = value; }, [], event(), undefined, testServices({ orchDir: tempOrchDir("herdr-hardening-"), settings: null }).settings);
