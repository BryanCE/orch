...
describe("orch settings notify", () => {
  test("records a sink with the field that sink declares", async () => {
    ...
    expect(fileSettingsManager(root).current().notify).toEqual([{ id: "webhook", url: "https://example.test/hook" }]);
  });
  ...
  test("the notify row lists every sink, the states it may fire on, and the fields each carries", async () => {
    ...
    expect(row.read(fileSettingsManager(root).current())).toEqual([{ id: "webhook", url: "https://example.test/hook", on: ["done"] }]);
  });
