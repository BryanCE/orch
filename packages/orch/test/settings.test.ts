...
describe("loadSettings", () => {
  ...
  test("parses every supported settings section", () => {
    ...
    expect(fileSettingsManager(directory).current()).toEqual({
      ...
      hosts: { gpu1: { dest: "bryan@gpu1" } },
      spaces: { wD: "Design" },
      daemon: { tcp_port: 4321, idle_shutdown_minutes: 30, outbox_drain_ms: 1000, liveness_poll_ms: 5_000, report_timeout_ms: 500, bridge_reconnect_ms: 1000, outbox_max_attempts: 120 },
      doctor: { unclaimed_after_ms: 123_456 },
      tiling: { first_split: "columns" },
      logging: { level: "debug" },
