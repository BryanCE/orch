...
export const Tooltip = ({
  ...
  const handleMouseLeave = () => {
    // Clear pending timeout if leaving before delay completes
  ...
  const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Toggle visibility on click - works on all devices
    if (isVisible) {
      setIsVisible(false);
      setMouse({ x: 0, y: 0 });
      setPosition({ x: 0, y: 0 });
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className="pointer-events-none absolute z-50 min-w-[15rem] overflow-hidden rounded-md border bg-popover shadow-md"
            style={{
              top: position.y,
              left: position.x,
            }}
