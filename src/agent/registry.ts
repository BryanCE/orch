...
export type Placement = Pick<SpawnedRecord, "pane" | "backend" | "workspace" | "handle" | "cwd" | "worktree" | "branch">;
 ...
 *  here reads as "no workspace", which opens the workspace wall. */
export function placementOf(orchDir: string, key: string): Placement | null {
  const record = selectSpawnedRecord(orchDir, key);
  if (!record) return null;
  const { pane, backend, workspace, handle, cwd, worktree, branch } = record;
  return { pane, backend, workspace, handle, cwd, worktree, branch };
}
