...
describe("commands/spawn", () => {
  ...
  test("preserves the existing named-spawn path", () => expect(parseSpawnFlags(["2", "--name", "worker", "--agent", "claude", "--backend", "headless", "--json"])).toMatchObject({ positional: ["2"], namePrefix: "worker", adapterFlag: "claude", backendFlag: "headless", json: true, unknownFlags: [] }));
