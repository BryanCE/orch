...
import { assertNameFree, liveNamedRecords, nextNameIndex } from "../src/policy/name.ts";
...
function seedLiveAgent(orchDir: string, key: string, name: string, workspace: string): void {
  recordSpawned(key, { name, workspace, handle: key });
...
describe("spawn name numbering", () => {
  ...
  test("continues past the highest live index so a live fleet is grown, not collided with", () => {
    ...
    expect(liveNamedRecords("recon", "w1").map((record) => record.name).sort()).toEqual(["recon-1", "recon-3"]);
  ...
  test("a dead agent frees its name and its index", () => {
    const orchDir = tempOrchDir();
    recordSpawned("herdr~w1~dead", { name: "recon-1", workspace: "w1", handle: "herdr~w1~dead" });
